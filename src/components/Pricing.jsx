        ...
        {pricingOptions.map((option, index) => (
            <div key={index} className="w-full sm:w-1/2 lg:w-1/3 p-2 ">
                <div className=" p-10 border border-neutral-700 rounded-xl">
                    <p className="flex text-3xl md:text-4xl mb-8">
                        {option.title}
                        {option.title === "Pro" && (
                           <span className="bg-gradient-to-r from-orange-500 to-orange-900 text-transparent bg-clip-text text-xl mb-4 ml-2 mt-1 sm:flex">(Most Popular)</span>
                        )}
                    </p>
                    <p className="mb-8">
                        <span className="text-3xl sm:text-5xl mt-6 mr-2">{option.price}</span>
